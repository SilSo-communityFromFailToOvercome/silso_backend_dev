...
class CourtService {
  ...
  Future<String> createCourtSession({
    ...
    try {
      ...
      final courtData = {
        ...
        'timeLeft': const Duration(hours: 2).inMilliseconds, // 2 hours in milliseconds
  ...
  // Update time left for a court session
  Future<void> updateTimeLeft(String courtId, Duration timeLeft) async {
    try {
      await _courtsCollection.doc(courtId).update({
      ...
      throw Exception('Failed to update time left: ${e.toString()}');
    }
  }
