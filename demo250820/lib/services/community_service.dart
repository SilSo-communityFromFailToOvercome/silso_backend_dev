...
class CommunityService {
  ...
  // Get communities user has joined (real-time stream)
  Stream<List<Community>> getMyCommunitiesStream() {
    if (currentUserId == null) {
      return Stream.value([]);
    }
  ...
  // Get recommended communities stream (updates when user's joined communities change)
  Stream<List<Community>> getRecommendedCommunitiesStream() {
    if (currentUserId == null) {
      return Stream.value([]);
    }

    return getMyCommunitiesStream().asyncMap((userCommunities) async {
      try {
        final userInterests = await getUserInterests();
        final allCommunities = await getAllCommunities();
        
        if (allCommunities.isEmpty) return <Community>[];
        ...
        if (availableCommunities.isEmpty) return <Community>[];

        if (userInterests.isEmpty) {
          // Return general recommendations based on member count
          availableCommunities.sort((a, b) => b.memberCount.compareTo(a.memberCount));
          return availableCommunities.take(5).toList();
        }
        ...
        debugPrint('Error getting recommended communities stream: $e');
        return <Community>[];
      }
    });
  }
